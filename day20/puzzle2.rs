...
fn build_graph(
    ...
    for level in 0..recursion_level {
        for (node_name, edges) in graph.iter() {
            ...
            let updated_node_name: String =
                match node_name.contains("AA") || node_name.contains("ZZ") {
                    true => node_name.to_string(),
                    false => extend_by(&node_name[0..4], &level.to_string()),
                };
...
fn main() {
    ...
    let graph = build_graph(&content, &portals, 26);
    println!("part 2: {}", dijkstra(&graph, "AA"));
}
