...
fn get_intersection_point(segment1: &Segment, segment2: &Segment) -> Option<(i64, i64)>
{
    ...
    if x >= seg1_x_min && x <= seg1_x_max && x >= seg2_x_min && x <= seg2_x_max && y >= seg1_y_min && y <= seg1_y_max && y >= seg2_y_min && y <= seg2_y_max {
        return Some((x, y));
    }
    None
}
...
fn main() {
    ...
    for line1_segments in all[0].iter() {
        for line2_segments in all[1].iter() {
            let ip = get_intersection_point(&line1_segments, &line2_segments);
            if ip.is_some() {
                let intersection = ip.unwrap();
                //println!("{:?}", ip);
                let distance = intersection.0.abs() + intersection.1.abs();
                //println!("{}: {:?}\n{:?}\n{:?}\n", distance, intersection, line1_segments, line2_segments);
